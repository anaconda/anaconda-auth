...
def install_env_manager() -> bool:
    ...
    if returncode != 0:
        logger.debug("Failed to install %s: %s", ENV_MANAGER_PACKAGE, stderr)
        return False
    return True
...
def register_org(org_name: str) -> bool:
    ...
    stdout, stderr, returncode = run_command(
        "env-log", "register", "--organization-name", org_name
    )
    if returncode != 0:
        logger.debug("Failed to register org '%s': %s", org_name, stderr)
        return False
    return True
